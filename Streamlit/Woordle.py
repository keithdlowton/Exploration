...
from wordler.utils import (
    color_background_red,
    convert_guess_and_hints,
    get_logo,
    get_past_solutions,
    is_valid_word,
    keep_valid_guesses,
...
st.sidebar.caption(
    "A [WORDLE](https://www.nytimes.com/games/wordle/index.html) ch🟨🟩t sh🟨🟨t made by [Siavash Yasini](https://www.linkedin.com/in/siavash-yasini/)."
)
st.sidebar.caption(
    "Look behind the scenes of Wordler [here](https://blog.streamlit.io/the-ultimate-wordle-cheat-sheet/)."
)
...
n_suggestions = st.sidebar.slider(
    "Suggestion list limit (0 shows all)",
    0,
    30,
    value=10,
)
...
mark_past_solutions = st.sidebar.checkbox(
    "Mark past solutions", value=True, help="When checked, previous WORDLE solutions will be highlighted in red."
)
...
if st.session_state["FormSubmitter:form-submit"]:
    ...
    if mark_past_solutions:
        suggestions_df = suggestions_df.style.applymap(
            color_background_red, subset=["word"], word_list=get_past_solutions()
        )
